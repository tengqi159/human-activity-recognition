...
class conv_loss_block(nn.Module):
    def __init__(self, channel_in, channel_out, height_width, kernel, stride, bias,numlayer):
        ...
        decode_t_list = [37632, 36096, 16128, 2560]#128,256,384
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(channel_in, channel_out, kernel_size=kernel, stride=stride, padding=0, bias=self.bias),
            nn.BatchNorm2d(channel_out),
            nn.LeakyReLU(inplace=True),

        )

        self.avg_pool = nn.MaxPool2d((4,1), padding=(1,0),stride=(1,1))
        ...
        self.conv_loss = nn.Sequential(
            nn.Conv2d(channel_out, channel_out, kernel_size=(4,1), stride=(2,1), padding=(1,1), bias=False),
...
class convnet(nn.Module):
    def __init__(self, input_ch, output_ch, height, num_layers, num_hiden, num_classes,lr):
        ...
        self.layers = nn.ModuleList(
            [conv_loss_block(self.input_ch, 128, self.height, kernel=(6,1), stride=(2,1), bias=False,numlayer=num_layers)])
        self.layers.extend(
            [conv_loss_block(128, 256, self.height, kernel=(6,1), stride=(2,1), bias=False,numlayer=num_layers),
             conv_loss_block(256, 384, self.height, kernel=(6,2), stride=(2,1), bias=False,numlayer=num_layers)
             ])


        self.layer_out = nn.Linear(16128, num_classes)
...
def train(train_loader, test_x_path, test_y_path,test_error):
    ...
    if epoch % 1 == 0:
        ...
        model.train()
    # np.save('./matplotlib_picture/WISDM_error/pred_test.npy', test_error)

if __name__ == '__main__':

    model = convnet(input_ch=1, output_ch=64 * 2, height=128 * 2, num_layers=3, num_hiden=100, num_classes=6,lr=0.001)
    model.cuda()
    print(model)
